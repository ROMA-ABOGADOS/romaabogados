...
import { pageContabilidadQuery } from "@/sanity/queries";
...
export function ContabilidadPage() {
  ...
  const sanityDoc = useSanityDocument<any>(pageContabilidadQuery, null);

  const heroImageSrc = sanityDoc?.heroImage
    ? getSanityImageUrl(sanityDoc.heroImage, "/jhon-contabilidad.webp")
    : "/jhon-contabilidad.webp";

  const heroSubtitle =
    sanityDoc?.heroSubtitle ||
    "Nos encargamos de tus libros electrónicos, SIRE, declaraciones mensuales y planillas laborales. Cero multas, cero contingencias.";

  const activeMainFeatures = (sanityDoc?.mainFeatures && sanityDoc.mainFeatures.length > 0)
    ? sanityDoc.mainFeatures.map((item: any, idx: number) => ({
        ...item,
        icon: mainFeatures[idx % mainFeatures.length].icon,
      }))
    : mainFeatures;

  const activeAdditionalServices = (sanityDoc?.additionalServices && sanityDoc.additionalServices.length > 0)
    ? sanityDoc.additionalServices.map((item: any, idx: number) => ({
        ...item,
        icon: additionalServices[idx % additionalServices.length].icon,
      }))
    : additionalServices;

  const activeLaborFeatures = (sanityDoc?.laborFeatures && sanityDoc.laborFeatures.length > 0)
    ? sanityDoc.laborFeatures.map((item: any, idx: number) => ({
        ...item,
        icon: planillasFeatures[idx % planillasFeatures.length].icon,
      }))
    : planillasFeatures;
  ...
  return (
    <SiteLayout>
        ...
        <div className="w-full max-w-xl md:max-w-2xl lg:max-w-3xl flex flex-col justify-center text-left">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.35 }}>
            ...
            >
              ...
              <span className="text-[#C5A572]">y Tributación</span>
            </motion.h1>
            {/* Subtitle */}
            <motion.p
              initial={{ opacity: 0, y: 20 }}
      ...
      <section id="servicios-incluidos" className="py-20 lg:py-28 bg-[#FAF8F5]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="inline-block text-[#C5A572] font-semibold text-sm tracking-wider uppercase mb-4">Servicios Incluidos</span>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#183D2F]">
              Todo lo que necesitas para estar <span className="text-[#C5A572]">al día con SUNAT</span>
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {activeMainFeatures.map((feature: any, i: number) => {
      ...
      <section id="contabilidad-digital" className="py-20 lg:py-28 bg-[#FAF8F5]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

          {/* Header */}
          <ScrollReveal>
            <div className="text-center mb-12">
              <span className="inline-block text-[#C5A572] font-semibold text-sm tracking-wider uppercase mb-3">Plan Promocional</span>
              <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#183D2F] leading-tight mb-3">
                CONTABILIDAD <span className="text-[#C5A572]">DIGITAL</span>
              </h2>
              <p className="text-[#364A41] text-lg max-w-xl mx-auto">Tu empresa merece respaldo real</p>
            </div>
          </ScrollReveal>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10">
            ...
            <ScrollReveal delay={0.1}>
              <div className="relative bg-white rounded-2xl shadow-xl shadow-[#183D2F]/5 border border-[#E8E2D5] overflow-hidden">
                {/* Gold ribbon */}
                <div className="bg-[#183D2F] text-[#C5A572] text-center py-2 text-sm font-bold tracking-wider uppercase">
                  Precio Promocional
                </div>
                <div className="p-8 text-center">
                  <p className="text-[#364A41] text-sm uppercase tracking-wider mb-2">Desde</p>
                  <div className="flex items-baseline justify-center gap-1 mb-1">
                    <span className="text-[#183D2F]/60 text-lg font-semibold">S/</span>
                    <span className="text-5xl font-bold text-[#183D2F]">250</span>
                  </div>
                  <p className="text-[#364A41] text-sm mb-6">al mes</p>
                  <p className="text-xs text-[#364A41]/70 mb-6">Modalidad Promocional</p>
                  <a
                    ...
                    className="inline-flex items-center justify-center gap-2.5 w-full bg-[#C5A572] hover:bg-[#B39360] text-[#112C22] px-7 py-4 rounded-xl text-base font-bold transition-all shadow-lg shadow-[#C5A572]/25 active:scale-[0.98]"
                  >
            ...
            <div className="mt-10 bg-amber-50 border border-amber-200 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-start gap-4">
              ...
              <div>
                ...
                <p className="text-amber-700 text-sm leading-relaxed">
                  Evita multas y contingencias: No dejes tus declaraciones al azar. Una sola infracción o multa de SUNAT puede costarte hasta <strong>S/ 5,500</strong> o más. En <strong>ROMA ABOGADOS</strong> nos encargamos de proteger tu patrimonio.
                </p>
              </div>
            </div>
          </ScrollReveal>
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
              ...
              <Mail className="w-5 h-5 text-navy" />
                ...
                <div>
                  ...
                  <p className="font-bold text-navy text-sm">contacto@romaabogados.pe</p>
                </div>
              </a>
            </div>
          </ScrollReveal>
      ...
      {/* Stats & Price Panel */}
      <SectionDivider from="#FAF8F5" to="#FAF8F5" />
      <section id="estadisticas-beneficios" ref={statsRef} className="py-20 lg:py-28 bg-[#FAF8F5]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div
              ...
              className="bg-[#183D2F] rounded-2xl p-8 lg:p-10 text-white relative overflow-hidden shadow-xl"
            >
              <div className="absolute -top-20 -right-20 w-40 h-40 bg-[#C5A572]/20 rounded-full blur-2xl" />
              <div className="absolute -bottom-20 -left-20 w-40 h-40 bg-[#22523F]/30 rounded-full blur-2xl" />
              <div className="relative">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-12 h-12 bg-[#C5A572]/20 rounded-xl flex items-center justify-center">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {stats.map((stat) => (
                    <div key={stat.label} className="text-center bg-white/5 rounded-xl p-4 border border-white/10">
                      <div className="text-2xl font-bold text-[#C5A572]">{stat.value}</div>
            ...
            <div>
              <motion.div
                ...
                transition={{ duration: 0.6, delay: 0.2 }}
              >
                <h3 className="text-2xl font-bold text-[#183D2F] mb-6">Beneficios de tercerizar tu contabilidad</h3>
                  ...
                  {benefits.map((b, i) => (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <span className="text-[#364A41] leading-relaxed">{b}</span>
                      </div>
                    </ScrollReveal>
                  ))}
                </div>
      ...
      {/* Additional Services */}
      <SectionDivider from="#FAF8F5" to="#FAF8F5" />
      <section id="servicios-complementarios" className="py-20 lg:py-28 bg-[#FAF8F5]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="inline-block text-[#C5A572] font-semibold text-sm tracking-wider uppercase mb-4">Servicios Complementarios</span>
            <h2 className="text-3xl sm:text-4xl font-bold text-[#183D2F]">
              Más de lo que esperas
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {activeAdditionalServices.map((s: any, i: number) => {
              const Icon = s.icon;
              return (
                <ScrollReveal
                  key={s.title || i}
                  delay={0.1 * i}
                  className="bg-white rounded-2xl p-6 lg:p-8 border border-[#E8E2D5] text-center shadow-sm"
                >
                  <div className="w-14 h-14 bg-[#C5A572]/15 rounded-xl flex items-center justify-center mx-auto mb-4">
                    <Icon className="w-7 h-7 text-[#183D2F]" />
                  </div>
                  <h3 className="text-lg font-bold text-[#183D2F] mb-2">{s.title}</h3>
                  <p className="text-sm text-[#364A41] leading-relaxed mb-4">{s.desc}</p>
                  <span className="inline-block bg-[#C5A572]/15 text-[#183D2F] font-bold text-sm px-3 py-1 rounded-full">{s.price}</span>
                </ScrollReveal>
              );
            })}
          </div>
      ...
      {/* Planillas y Laboral */}
      <SectionDivider from="#FAF8F5" to="#FAF8F5" />
      <section id="planillas-laboral" className="py-20 lg:py-28 bg-[#FAF8F5]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="inline-block text-[#C5A572] font-semibold text-sm tracking-wider uppercase mb-4">Servicio Especializado</span>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#183D2F]">
              Planillas y Laboral <span className="text-[#C5A572]">Tercerización total</span>
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {activeLaborFeatures.map((feature: any, i: number) => {
              const Icon = feature.icon;
              return (
                <ScrollReveal
                  key={feature.title || i}
      ...
      {/* CTA */}
      <SectionDivider from="#FAF8F5" to="#183D2F" />
      <section id="contacto" className="py-20 lg:py-28 bg-gradient-to-r from-[#183D2F] to-[#112C22]">
